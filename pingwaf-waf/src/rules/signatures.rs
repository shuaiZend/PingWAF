...
fn builtin_patterns() -> Vec<SignaturePattern> {
    ...
    // out below.
    {
    let mut push = |id: &str, cat: AttackCategory, sev: u8, desc: &str, needle: &'static str| {
    ...
    // ---- Path traversal ----
    push("PT-001", AttackCategory::PathTraversal, 4, "directory traversal (../)", "../");
    push("PT-002", AttackCategory::PathTraversal, 4, "directory traversal (..\\)", "..\\");
    push("PT-003", AttackCategory::PathTraversal, 5, "etc/passwd access", "/etc/passwd");
    push("PT-004", AttackCategory::PathTraversal, 5, "etc/shadow access", "/etc/shadow");
    push("PT-005", AttackCategory::PathTraversal, 4, "proc/self enumeration", "/proc/self");
    push("PT-006", AttackCategory::PathTraversal, 4, "windows system32 access", "windows\\system32");
    push("PT-007", AttackCategory::PathTraversal, 4, "windows system32 access (forward slash)", "/windows/system32");
    push("PT-008", AttackCategory::PathTraversal, 3, "htaccess probe", ".htaccess");
    push("PT-009", AttackCategory::PathTraversal, 3, "htpasswd probe", ".htpasswd");
    push("PT-010", AttackCategory::PathTraversal, 3, "web.config probe", "web.config");
    push("PT-011", AttackCategory::PathTraversal, 3, "boot.ini probe", "boot.ini");
    push("PT-012", AttackCategory::PathTraversal, 4, "double-encoded traversal", "....//");

    // ---- Command injection ----
    push("CI-001", AttackCategory::CommandInjection, 5, "command substitution $(whoami)", "$(whoami)");
    push("CI-002", AttackCategory::CommandInjection, 5, "command substitution $(id)", "$(id)");
    push("CI-003", AttackCategory::CommandInjection, 4, "backtick command", "`id`");
    push("CI-004", AttackCategory::CommandInjection, 4, "backtick command (whoami)", "`whoami`");
    push("CI-005", AttackCategory::CommandInjection, 4, "shell pipe to cat", "|cat");
    push("CI-006", AttackCategory::CommandInjection, 4, "shell pipe to ls", "|ls");
    push("CI-007", AttackCategory::CommandInjection, 4, "shell pipe to wget", "|wget");
    push("CI-008", AttackCategory::CommandInjection, 4, "shell pipe to curl", "|curl");
    push("CI-009", AttackCategory::CommandInjection, 4, "semicolon chained ls", ";ls");
    push("CI-010", AttackCategory::CommandInjection, 4, "semicolon chained cat", ";cat");
    push("CI-011", AttackCategory::CommandInjection, 4, "semicolon chained id", ";id");
    push("CI-012", AttackCategory::CommandInjection, 4, "semicolon chained wget", ";wget");
    push("CI-013", AttackCategory::CommandInjection, 4, "semicolon chained curl", ";curl");
    push("CI-014", AttackCategory::CommandInjection, 5, "&& chained wget", "&&wget");
    push("CI-015", AttackCategory::CommandInjection, 5, "&& chained curl", "&&curl");
    push("CI-016", AttackCategory::CommandInjection, 5, "|| chained curl", "||curl");
    push("CI-017", AttackCategory::CommandInjection, 5, "|| chained wget", "||wget");
    push("CI-018", AttackCategory::CommandInjection, 4, "IFS variable", "${ifs}");
    push("CI-019", AttackCategory::CommandInjection, 5, "/bin/sh invocation", "/bin/sh");
    push("CI-020", AttackCategory::CommandInjection, 5, "/bin/bash invocation", "/bin/bash");
    push("CI-021", AttackCategory::CommandInjection, 5, "interactive bash", "bash -i");
    push("CI-022", AttackCategory::CommandInjection, 5, "cmd.exe invocation", "cmd.exe");
    push("CI-023", AttackCategory::CommandInjection, 5, "powershell invocation", "powershell");
    push("CI-024", AttackCategory::CommandInjection, 4, "nc reverse shell", "nc -e");
    push("CI-025", AttackCategory::CommandInjection, 4, "newline before /bin/", "\n/bin/");

    // ---- SSRF ----
    push("SSRF-001", AttackCategory::Ssrf, 5, "AWS metadata endpoint", "169.254.169.254");
    push("SSRF-002", AttackCategory::Ssrf, 5, "GCP metadata host", "metadata.google");
    push("SSRF-003", AttackCategory::Ssrf, 5, "AWS task metadata", "169.254.170.2");
    push("SSRF-004", AttackCategory::Ssrf, 4, "loopback IPv6", "[::1]");
    push("SSRF-005", AttackCategory::Ssrf, 4, "loopback IPv4", "127.0.0.1");
    push("SSRF-006", AttackCategory::Ssrf, 3, "wildcard IPv4", "0.0.0.0");
    push("SSRF-007", AttackCategory::Ssrf, 4, "localhost with port", "localhost:");
    push("SSRF-008", AttackCategory::Ssrf, 5, "file:// scheme", "file://");
    push("SSRF-009", AttackCategory::Ssrf, 5, "gopher:// scheme", "gopher://");
    push("SSRF-010", AttackCategory::Ssrf, 5, "dict:// scheme", "dict://");
    push("SSRF-011", AttackCategory::Ssrf, 4, "ldap:// scheme", "ldap://");
    push("SSRF-012", AttackCategory::Ssrf, 4, "ftp:// scheme", "ftp://");

    // ---- Deserialization ----
    push("DZ-001", AttackCategory::Deserialization, 5, "Java serialized stream", "aced0005");
    push("DZ-002", AttackCategory::Deserialization, 5, "Python __reduce__", "__reduce__");
    push("DZ-003", AttackCategory::Deserialization, 5, "Node.js prototype pollution", "__proto__");
    push("DZ-004", AttackCategory::Deserialization, 4, "Python class introspection", "__class__");
    push("DZ-005", AttackCategory::Deserialization, 4, "PHP magic __wakeup", ":__wakeup");
    push("DZ-006", AttackCategory::Deserialization, 4, "PHP magic __destruct", ":__destruct");
    push("DZ-007", AttackCategory::Deserialization, 4, "PHP magic __toString", ":__tostring");
    push("DZ-008", AttackCategory::Deserialization, 5, "Java Runtime.exec", "runtime.exec");
    push("DZ-009", AttackCategory::Deserialization, 5, "Java ProcessBuilder", "processbuilder");
    push("DZ-010", AttackCategory::Deserialization, 4, "PHP unserialize call", "unserialize(");
    push("DZ-011", AttackCategory::Deserialization, 4, "YAML unsafe_load", "yaml.unsafe_load");

    // ---- XXE ----
    push("XXE-001", AttackCategory::Xxe, 5, "DOCTYPE declaration", "<!doctype");
    push("XXE-002", AttackCategory::Xxe, 5, "ENTITY declaration", "<!entity");
    push("XXE-003", AttackCategory::Xxe, 5, "SYSTEM file: entity", "system \"file:");
    push("XXE-004", AttackCategory::Xxe, 5, "SYSTEM file: entity (single-quoted)", "system 'file:");
    push("XXE-005", AttackCategory::Xxe, 4, "PUBLIC identifier", "public \"-//");

    // ---- CRLF ----
    push("CRLF-001", AttackCategory::CrlfInjection, 4, "raw CRLF in value", "\r\n");
    push("CRLF-002", AttackCategory::CrlfInjection, 4, "raw CR in value", "\r");

    // ---- Template injection ----
    push("TI-001", AttackCategory::TemplateInjection, 4, "Jinja/Twig double brace", "{{");
    push("TI-002", AttackCategory::TemplateInjection, 4, "Jinja statement block", "{%");
    push("TI-003", AttackCategory::TemplateInjection, 4, "EL/Shell expression", "${");
    push("TI-004", AttackCategory::TemplateInjection, 4, "Ruby/JS template hash", "#{");
    push("TI-005", AttackCategory::TemplateInjection, 4, "EJS/JSP expression", "<%=");
    push("TI-006", AttackCategory::TemplateInjection, 5, "Log4Shell JNDI lookup", "${jndi:");
    push("TI-007", AttackCategory::TemplateInjection, 5, "Jinja self reference", "{{self");
    push("TI-008", AttackCategory::TemplateInjection, 5, "Jinja config leak", "{{config");

    // ---- SQL injection (literal needles; libinjection catches the rest) ----
    push("SQL-001", AttackCategory::SqlInjection, 5, "UNION SELECT", "union select");
    push("SQL-002", AttackCategory::SqlInjection, 5, "UNION ALL SELECT", "union all select");
    push("SQL-003", AttackCategory::SqlInjection, 4, "SLEEP function", "sleep(");
    push("SQL-004", AttackCategory::SqlInjection, 4, "BENCHMARK function", "benchmark(");
    push("SQL-005", AttackCategory::SqlInjection, 4, "pg_sleep function", "pg_sleep(");
    push("SQL-006", AttackCategory::SqlInjection, 4, "waitfor delay", "waitfor delay");
    push("SQL-007", AttackCategory::SqlInjection, 4, "load_file function", "load_file(");
    push("SQL-008", AttackCategory::SqlInjection, 4, "INTO OUTFILE", "into outfile");
    push("SQL-009", AttackCategory::SqlInjection, 4, "INTO DUMPFILE", "into dumpfile");
    push("SQL-010", AttackCategory::SqlInjection, 3, "information_schema probe", "information_schema");
    push("SQL-011", AttackCategory::SqlInjection, 3, "sqlite_master probe", "sqlite_master");
    push("SQL-012", AttackCategory::SqlInjection, 3, "pg_catalog probe", "pg_catalog");
    push("SQL-013", AttackCategory::SqlInjection, 4, "group_concat aggregation", "group_concat(");
    push("SQL-014", AttackCategory::SqlInjection, 4, "concat_ws aggregation", "concat_ws(");
    push("SQL-015", AttackCategory::SqlInjection, 4, "extractvalue function", "extractvalue(");
    push("SQL-016", AttackCategory::SqlInjection, 4, "updatexml function", "updatexml(");
    push("SQL-017", AttackCategory::SqlInjection, 4, "DROP TABLE", "drop table");
    push("SQL-018", AttackCategory::SqlInjection, 4, "TRUNCATE TABLE", "truncate table");
    push("SQL-019", AttackCategory::SqlInjection, 4, "exec xp_cmdshell", "xp_cmdshell");

    // ---- XSS (literal needles; detect_xss catches structured payloads) ----
    push("XSS-001", AttackCategory::Xss, 5, "<script tag", "<script");
    push("XSS-002", AttackCategory::Xss, 5, "</script tag", "</script");
    ...
    push("XSS-007", AttackCategory::Xss, 5, "javascript: URI", "javascript:");
    push("XSS-008", AttackCategory::Xss, 5, "vbscript: URI", "vbscript:");
    push("XSS-009", AttackCategory::Xss, 4, "data:text/html URI", "data:text/html");
    push("XSS-010", AttackCategory::Xss, 4, "onerror handler", "onerror=");
    push("XSS-011", AttackCategory::Xss, 4, "onload handler", "onload=");
    push("XSS-012", AttackCategory::Xss, 4, "onmouseover handler", "onmouseover=");
    push("XSS-013", AttackCategory::Xss, 4, "onclick handler", "onclick=");
    push("XSS-014", AttackCategory::Xss, 4, "onfocus handler", "onfocus=");
    push("XSS-015", AttackCategory::Xss, 3, "document.cookie access", "document.cookie");
    push("XSS-016", AttackCategory::Xss, 3, "window.location assignment", "window.location");
    push("XSS-017", AttackCategory::Xss, 4, "CSS expression()", "expression(");
...
const SQL_KEYWORDS: &[&str] = &[
    "select", "union", "all", "distinct", "from", "where", "and", "or", "not", "in", "like",
    "between", "is", "null", "having", "group", "order", "by", "limit", "offset", "asc", "desc",
    "insert", "into", "values", "update", "set", "delete", "drop", "alter", "create", "truncate",
    "table", "database", "schema", "user", "version", "current_user", "session_user",
    "join", "inner", "left", "right", "outer", "on", "as", "case", "when", "then", "else", "end",
    "exists", "any", "some", "with", "recursive", "cast", "convert", "exec", "execute",
    "begin", "commit", "rollback", "savepoint", "grant", "revoke", "show", "describe",
    "true", "false",
];

/// SQL functions that are strong attack indicators on their own.
const SQL_FUNCTIONS: &[&str] = &[
    "sleep", "benchmark", "waitfor", "delay", "pg_sleep", "load_file", "outfile", "dumpfile",
    "extractvalue", "updatexml", "concat", "concat_ws", "group_concat", "char", "ascii", "ord",
    "hex", "unhex", "substring", "substr", "mid", "left", "right", "length", "count", "sum",
    "avg", "min", "max", "rand", "floor", "ceil", "round", "if", "ifnull", "nullif", "coalesce",
    "version", "user", "database", "schema", "current_user", "session_user", "system_user",
    "row_number", "rank", "dense_rank", "xp_cmdshell", "sp_executesql", "make_set", "elt",
    "find_in_set", "regexp", "rlike",
];
...
static SQLI_COMMENT_TERM: Lazy<Regex> = Lazy::new(|| {
...
static SQLI_INTO_FILE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\binto\s+(?:out|dump)file\b").unwrap()
});
...
static XSS_EVENT_HANDLER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)\son[a-z]+\s*=").unwrap()
});
...
static XSS_HTML_COMMENT_BREAK: Lazy<Regex> = Lazy::new(|| {
    ...
    Regex::new(r"(?i)<\s*/\s*(?:style|title|textarea|noscript|comment)\b").unwrap()
});
...
mod tests {
    ...
    fn engine_finds_path_traversal() {
        ...
        assert!(hits.iter().any(|h| h.category == AttackCategory::PathTraversal));
    }
    ...
    fn detect_sqli_union_select() {
        let (hit, fp) = detect_sqli("1 UNION SELECT username, password FROM users");
        assert!(hit);
